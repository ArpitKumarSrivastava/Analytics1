...
frequentItems = eclat (Groceries, parameter = list(supp = 0.01, minlen= 1, maxlen = 5))   #eclat find out the frequently purchased items || supp is minimium threshold level i.e. .01*10000 = 100 i am looking for items that are purchased atleast 100 times || minlen - i want to buy atleast 1 item  ||  maxlen given the maximum items purchased in single transactions
inspect(frequentItems[1:10])    # looks like whole milk , butter milk etc is frequently purchased
frequentItems
inspect(frequentItems[10:100])
#inspect(frequentItems[100:122])

#Descending Sort frequent items by count : 1 to 25 itemsets
inspect(sort (frequentItems, by="count", decreasing=TRUE)[1:25])      # whole milk is purchased maximum times
...
# Create ASSOCIATION rules (X => Y) and the relationship between items

#parameters are min filter conditions 
# apriori is used to create rules || rules are created X => Y
rules = apriori(Groceries, parameter = list(supp = 0.005, conf = 0.5, minlen=2))    # supp or support is 50% confidence
rules
inspect (rules[1:5])
