...
#Model1
fit1 = rpart(buy ~ gender, data = students1)
fit1 = rpart(buy ~ gender, data=students1,minsplit=4, minbucket=2)   #rpart is used for spliting of the decision tree
...
library(rpart.plot)
?rpart.plot
rpart.plot(fit1, main='Classification Tree', nn=T, type=4, extra=104)    #nn=T i.e. we need node at T

fit1
# now passing a new data as female gender
predict(fit1, newdata = data.frame(gender='Female'))
# by giving type=c('class"), i am classifying my decision for the particular new data
...
# Model2
#library(rpart)
fit2 = rpart(buy ~ gender + married, data=students2, minsplit=12)    # atleast at the decision node there should be atleast 12 records for split, otherwise dont split
...
prp(fit2, main="An Example",
    type=5, fallen=T, branch=.3, round=0, leaf.round=9,
...
head(cbind(students2, nodeno=rownames(fit2$frame) [ fit2$where]))
printcp(fit2)
...
#Lets see CP: Complexity parameter
plotcp(fit3b)
printcp(fit3b)    # print the complexity parameter table
