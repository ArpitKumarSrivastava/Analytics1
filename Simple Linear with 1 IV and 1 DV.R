...
#Analysis of the solution given above : from bottom
# F-statistics: if p<0.05 it means there ia linear model that exist between Y and atleast one X
# R-squared: 0.991 that means 99.1% of weight (Y) is explained by height (X)
# when there is multiple variables, we use adjusted R-squared
# coefficient: (Y) = -87.51667 + 3.45*(X)
# for height, if p-value is less than 0.05, we retained the value for the prediction; in this case, we are taking height into consideration
# Residuals: different between predicted and the actual weights ranges from -1.7333 to 3.1167
